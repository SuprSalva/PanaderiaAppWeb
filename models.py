...
class Usuario(db.Model):
    ...
    actualizado_en    = db.Column(db.DateTime, nullable=False, default=datetime.datetime.now, onupdate=datetime.datetime.now)
...
class MateriaPrima(db.Model):
    ...
    unidades_presentacion = db.relationship('UnidadPresentacion', back_populates='materia', cascade='all, delete-orphan')
...
class UnidadPresentacion(db.Model):
    ...
    id_materia     = db.Column(db.Integer,      db.ForeignKey('materias_primas.id_materia', ondelete='CASCADE'), nullable=False)
...
class DetalleCompra(db.Model):
    ...
    id_unidad_presentacion  = db.Column(db.Integer, db.ForeignKey('unidades_presentacion.id_unidad', ondelete='SET NULL'), nullable=True)  # nuevo v2
...
class Receta(db.Model):
    ...
    unidad_rendimiento = db.Column(db.String(20), nullable=False, default='pza')
    ...
    detalles = db.relationship('DetalleReceta', back_populates='receta', cascade='all, delete-orphan')


class DetalleReceta(db.Model):
    __tablename__ = 'detalle_recetas'
    id_detalle_receta      = db.Column(db.Integer, primary_key=True, autoincrement=True)
    id_receta              = db.Column(db.Integer, db.ForeignKey('recetas.id_receta', ondelete='CASCADE'), nullable=False)
    id_materia             = db.Column(db.Integer, db.ForeignKey('materias_primas.id_materia'), nullable=False)
    id_unidad_presentacion = db.Column(db.Integer, db.ForeignKey('unidades_presentacion.id_unidad', ondelete='SET NULL'), nullable=True)  # nuevo v2
    cantidad_requerida     = db.Column(db.Numeric(12,4), nullable=False)
    cantidad_presentacion  = db.Column(db.Numeric(12,4))  # nuevo v2
...
class ProductoTerminado(db.Model):
    ...
    id_receta      = db.Column(db.Integer,     db.ForeignKey('recetas.id_receta', ondelete='SET NULL'), nullable=True)
    nombre         = db.Column(db.String(120), nullable=False)
    descripcion    = db.Column(db.Text)
...
class Produccion(db.Model):
    ...
    id_producto       = db.Column(db.Integer,    db.ForeignKey('productos_terminados.id_producto'), nullable=False)
    ...
    estado            = db.Column(db.Enum('pendiente','en_proceso','finalizado','cancelado'), nullable=False, default='pendiente')
    fecha_inicio      = db.Column(db.DateTime)
    fecha_fin_estimado= db.Column(db.DateTime)
    fecha_fin_real    = db.Column(db.DateTime)
    operario_id       = db.Column(db.Integer, db.ForeignKey('usuarios.id_usuario', ondelete='SET NULL'), nullable=True)
    observaciones     = db.Column(db.Text)
    creado_en         = db.Column(db.DateTime, nullable=False, default=datetime.datetime.now)
    creado_por        = db.Column(db.Integer, db.ForeignKey('usuarios.id_usuario', ondelete='SET NULL'), nullable=True)
...
class DetalleProduccion(db.Model):
    ...
    id_produccion       = db.Column(db.Integer, db.ForeignKey('produccion.id_produccion', ondelete='CASCADE'), nullable=False)
    id_materia          = db.Column(db.Integer, db.ForeignKey('materias_primas.id_materia'), nullable=False)
    cantidad_requerida  = db.Column(db.Numeric(12,4), nullable=False, default=0)
    cantidad_descontada = db.Column(db.Numeric(12,4), nullable=False, default=0)
...
class Venta(db.Model):
    ...
    total           = db.Column(db.Numeric(12,2), nullable=False, default=0)
    metodo_pago     = db.Column(db.Enum('efectivo','tarjeta','transferencia','otro'), nullable=False, default='efectivo')
    cambio          = db.Column(db.Numeric(10,2))
    requiere_ticket = db.Column(db.Boolean, nullable=False, default=False)
    estado          = db.Column(db.Enum('abierta','completada','cancelada'), nullable=False, default='abierta')
    vendedor_id     = db.Column(db.Integer, db.ForeignKey('usuarios.id_usuario'), nullable=False)
    id_corte        = db.Column(db.Integer, db.ForeignKey('cortes_diarios.id_corte', ondelete='SET NULL'), nullable=True)
    creado_en       = db.Column(db.DateTime, nullable=False, default=datetime.datetime.now)
...
class CorteDiario(db.Model):
    ...
    cerrado_por    = db.Column(db.Integer, db.ForeignKey('usuarios.id_usuario', ondelete='SET NULL'), nullable=True)
...
class SalidaEfectivo(db.Model):
    ...
    categoria      = db.Column(db.Enum('compra_insumos','servicios_utilities','mantenimiento','otros'), nullable=False, default='otros')
    descripcion    = db.Column(db.String(255),nullable=False)
    monto          = db.Column(db.Numeric(12,2), nullable=False)
    fecha_salida   = db.Column(db.Date,       nullable=False)
    estado         = db.Column(db.Enum('pendiente','aprobada','rechazada'), nullable=False, default='pendiente')
    id_corte       = db.Column(db.Integer,    db.ForeignKey('cortes_diarios.id_corte', ondelete='SET NULL'), nullable=True)
...
class Merma(db.Model):
    ...
    tipo_objeto    = db.Column(db.Enum('materia_prima','producto_terminado','lote_produccion'), nullable=False)
    id_referencia  = db.Column(db.Integer, nullable=False)
    cantidad       = db.Column(db.Numeric(12,4), nullable=False)
    unidad         = db.Column(db.String(20), nullable=False)
    causa          = db.Column(db.Enum(
                        'caducidad','quemado_horneado','caida_accidente',
                        'error_produccion','rotura_empaque','contaminacion','otro'
                    ), nullable=False)
    descripcion    = db.Column(db.Text)
    id_produccion  = db.Column(db.Integer, db.ForeignKey('produccion.id_produccion', ondelete='SET NULL'), nullable=True)
    registrado_por = db.Column(db.Integer, db.ForeignKey('usuarios.id_usuario'), nullable=False)
...
class LogSistema(db.Model):
    ...
    tipo            = db.Column(db.Enum(
                        'error','acceso','cambio_usuario','venta','compra',
                        'produccion','ajuste_inv','merma','solicitud',
                        'salida_efectivo','seguridad'
                      ), nullable=False)
    nivel           = db.Column(db.Enum('INFO','WARNING','ERROR','CRITICAL'), nullable=False, default='INFO')
...
class Sesion(db.Model):
    ...
    creado_en    = db.Column(db.DateTime, nullable=False, default=datetime.datetime.now)
    ultimo_acceso= db.Column(db.DateTime, nullable=False, default=datetime.datetime.now, onupdate=datetime.datetime.now)


class Ticket(db.Model):
    __tablename__ = 'tickets'
    id_ticket      = db.Column(db.Integer, primary_key=True, autoincrement=True)
    id_venta       = db.Column(db.Integer, db.ForeignKey('ventas.id_venta', ondelete='CASCADE'), nullable=False, unique=True)
